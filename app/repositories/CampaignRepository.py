...
class CampaignRepository:
    ...
    def get_campaign_list_metrics(self, campaign_ids, hm_review_sla_days: int,
                                  stale_campaign_days: int) -> dict:
        """
        Every per-campaign figure the list endpoints show, for many campaigns in
        four aggregate queries instead of five per campaign.

        Returns {campaign_id: {candidate_count, shortlisted_count, selected_count,
        overdue_review, pipeline_stalled}}. Each value is computed with the same
        rule as its single-campaign counterpart above.
        """
        ...
        blank = {
            "candidate_count": 0, "shortlisted_count": 0, "selected_count": 0,
            "overdue_review": False, "pipeline_stalled": False,
        }
        out = {cid: dict(blank) for cid in ids}

        # counts per campaign, split by the stages the list cares about
        rows = self.db.execute(
            select(
                ...
                func.count(CampaignCandidate.id).filter(
                    CampaignCandidate.pipeline_stage == PipelineStage.SELECTED),
                func.max(CampaignCandidate.created_at),
            )
            .where(CampaignCandidate.campaign_id.in_(ids))
            .group_by(CampaignCandidate.campaign_id)
        ).all()

        stale_cutoff = datetime.now(timezone.utc) - timedelta(days=stale_campaign_days)
        for cid, total, shortlisted, selected, latest_added_at in rows:
            entry = out[cid]
            entry["candidate_count"] = total or 0
            entry["shortlisted_count"] = shortlisted or 0
            entry["selected_count"] = selected or 0
            # Stalled means "has candidates, but none added recently" — a
            # campaign with no candidates at all is not stalled.
            entry["pipeline_stalled"] = bool(latest_added_at and latest_added_at <= stale_cutoff)

        review_cutoff = datetime.now(timezone.utc) - timedelta(days=hm_review_sla_days)
        latest_entry = (
            select(
        ...
        overdue_rows = self.db.execute(
            select(CampaignCandidate.campaign_id, func.count(CampaignCandidate.id))
            .join(latest_entry, latest_entry.c.campaign_candidate_id == CampaignCandidate.id)
            .where(
                CampaignCandidate.campaign_id.in_(ids),
                CampaignCandidate.pipeline_stage == PipelineStage.HM_REVIEW,
                latest_entry.c.entered_at <= review_cutoff,
            )
            .group_by(CampaignCandidate.campaign_id)
        ).all()
        for cid, count in overdue_rows:
            out[cid]["overdue_review"] = (count or 0) > 0
