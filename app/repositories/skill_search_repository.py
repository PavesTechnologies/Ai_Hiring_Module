...
class SkillSearchRepository:
    ...
    def cross_campaign_candidates(
        self,
        *,
        skill_ids: list[UUID],
        accessible_campaign_ids,          # None = unrestricted (HR_ADMIN)
        ...
        """
        M11-E03-S04-T01/T02 — candidates holding ALL the given skills anywhere
        the caller can see, returned as one row per (candidate, campaign)
        appearance.

        Deduplication to one row per candidate happens in the service, because
        the per-campaign detail (stage and score in EACH campaign) is exactly
        what the results table shows — collapsing it in SQL would throw away
        ...
        # candidates matching ALL skills — same HAVING COUNT(DISTINCT) rule as
        # the single-campaign search, just not restricted to one campaign
        matching = (
        ...
        stmt = (
            select(
                ...
                HiringCampaign.status.label("campaign_status"),
            ...
            .where(CampaignCandidate.candidate_id.in_(select(matching.c.candidate_id)))
            .order_by(CampaignCandidate.composite_score.desc().nullslast())
            .limit(limit)
        )

        if accessible_campaign_ids is not None:
            stmt = stmt.where(CampaignCandidate.campaign_id.in_(accessible_campaign_ids))
        if min_composite_score is not None:
            stmt = stmt.where(CampaignCandidate.composite_score >= min_composite_score)
        if campaign_statuses:
            stmt = stmt.where(HiringCampaign.status.in_(campaign_statuses))
        if reached_stage is not None:
            # "reached this stage in at least one campaign" — applied per
            # appearance; the service decides whether the candidate qualifies.
            stmt = stmt.where(CampaignCandidate.pipeline_stage == reached_stage)
